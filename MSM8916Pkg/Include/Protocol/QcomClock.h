...
typedef struct clk *(EFIAPI *clk_get_t)(const CHAR8 *id);
...
typedef int(EFIAPI *clk_get_set_enable_t)(
    CHAR8 *id, unsigned long rate, BOOLEAN enable);
typedef int(EFIAPI *clk_reset_t)(struct clk *clk, enum clk_reset_action action);
